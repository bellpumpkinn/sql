...
select hire_date || ''  --날짜를 다 문자로변환
from employees;

select salary || ' '      --숫자를 문자로 변환.
from employees;
------------------------------------------------
--날짜를 문자로 바꿔주기.
--                           ↓폼의 모델이라해서 fm이라한다.
select to_char(sysdate, 'yyyy-mm-dd')
from dual;  --문자로 바꿔줌, 그리고 형식을 바꿔줌
...
--아래처럼 fx를 쓰면 포맷(형식)이 정확히 일치해야한다.
select last_name, hire_date
from employees
...
from dual; --형식에 맞추니깐 변환을 잘함.
--------------------------------------------------
--ETC

--nvl함수
select nvl(null, 0)  --조사할값이 null이면 0을 리턴.
from dual;

select job_id, nvl(commission_pct, 0) -- 검사할값과 기본값의 타입이 같아야함, 이유: 하나의칼럼을 구성하기때문
from employees;  --null로 표현하던부분이 0으로 바뀜.
