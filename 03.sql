...
select lower('SQL Course') --필드값 하나 있는 레코드가 파라미터로 들어간거.
from dual;
...
select concat('Hello', 'World') --파라미터 2개를 붙여서 리턴시켜줌.
from dual;
...
--글자를 뒤에서부터 뜯어내는 메소드
select substr('Hello', -1, 1)  --맨뒤가 -1, 왼쪽옆이 -2, 그옆이 -3
from dual;
...
--해당문자가 몇번인덱스에있는지 확인하는 메서드, sql은 인덱스가 1부터 시작.
select instr('Hello', 'l') --처음으로 발견된 l의 리턴값을 내보내고 끝냄.
from dual;
select instr('Hello', 'w')
from dual;

--왼쪽칸 채워서 맞추고 싶을때 쓰는 메서드
...
from dual;   --머리 꼬리에만 신경있음. 몸통은 신경안씀.
...
--다른형태
select trim(' Hello World ') --자동으로 빈칸삭제해줌.
from dual;
...
select last_name, salary, trunc(salary * 1.155) "New Salary", trunc(salary * 1.155) - salary "Increase"
from employees;
-----------------------------------------------------------
--날짜를 다루는 함수.
